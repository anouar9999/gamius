...
const MatchCardHeader = () => {
  return (
    <div className="bg-[#1e1f23] rounded-t-lg p-3 sm:p-4">
      <div className="flex flex-col sm:flex-row justify-between items-start">
        ...
        <div className="flex items-start mt-2 sm:mt-0">
          <div className="text-right mr-2">
            ...
            <p className="text-white font-semibold text-xs ">FORETOX EZR</p>
          </div>
...
const ScoreDisplay = ({ scoreA, scoreB }) => {
  return (
    <div className="flex items-center justify-center  rounded-lg px-3 py-1 sm:px-4 sm:py-2">
      <span className="text-white text-lg sm:text-2xl font-bold">{scoreA}</span>
      <span className="text-gray-400 text-sm sm:text-lg mx-1 sm:mx-2">-</span>
      <span className="text-white text-lg sm:text-2xl font-bold">{scoreB}</span>
    </div>
  );
};
...
const TeamDisplay = ({ title, name, logoSrc, isLosing }) => {
  ...
  return (
    <div className={`w-[40%] flex items-center ${isLosing ? 'opacity-50' : ''}`}>
      ...
      <img
        src={logoSrc}
        alt={`Team ${name}`}
        className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full ml-2 sm:ml-3 ${overlayClass}`}
      />
    </div>
  );
};
...
const PlayerAvatars = ({ count, isLosing }) => {
  const overlayClass = isLosing ? 'opacity-50' : '';
  return (
    <div className={`flex -space-x-2 w-[40%] ${isLosing ? 'justify-end' : ''}`}>
      {[
...
const MatchCardWithScore = () => {
  ...
  return (
    <div
      ...
      className="bg-[#1e1f23] rounded-lg text-white w-full max-w-sm sm:max-w-md mx-auto"
    >
      <MatchCardHeader />
      ...
      <div className="sm:p-4">
        <div className="flex flex-col">
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="inline-flex items-center px-1  bg-red-600 hover:bg-blue-300 rounded text-[10px] font-semibold text-white">
              Finished
            </span>
